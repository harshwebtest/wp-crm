    ...
    if (getUser[0]?.plan) {
        ...
        if (plan.allow_chatbot > 0) {
            ...
            if (chatbots.length > 0) {
                await Promise.all(chatbots.map((i) => runChatbot(i, incomingMsg, uid, senderNumber, toName)));
            }
        } else {
...
async function saveMessage(body, uid, type, msgContext) {
    try {
        ...
        const chatId = convertNumberToRandomString(body?.entry[0]?.changes[0]?.value?.contacts[0]?.wa_id, body?.entry[0]?.changes ? body?.entry[0]?.changes[0]?.value?.contacts[0]?.profile?.name : "NA")
        ...
        const actualMsg = {
            ...
            "senderName": body?.entry[0]?.changes ? body?.entry[0]?.changes[0]?.value?.contacts[0]?.profile?.name : "NA",
            "senderMobile": body?.entry[0]?.changes ? body?.entry[0]?.changes[0]?.value?.contacts[0]?.wa_id : "NA",
            "status": "",
            "star": false,
            "route": "INCOMING",
            "context": body?.entry[0]?.changes[0]?.value?.messages[0] ? body?.entry[0]?.changes[0]?.value?.messages[0]?.context : ""
        }
        ...
        if (chat.length < 1) {
            await query(`INSERT INTO chats (chat_id, uid, last_message_came, sender_name, sender_mobile, last_message, is_opened) VALUES (
            ?,?,?,?,?,?,?
        )`, [
                chatId,
                uid,
                userTimezone,
                body?.entry[0]?.changes ? body?.entry[0]?.changes[0]?.value?.contacts[0]?.profile?.name : "NA",
                body?.entry[0]?.changes ? body?.entry[0]?.changes[0]?.value?.contacts[0]?.wa_id : "NA",
                JSON.stringify(actualMsg),
                0
            ])
        } else {
            await query(`UPDATE chats SET last_message_came = ?, last_message = ?, is_opened = ? WHERE chat_id = ? AND uid = ?`, [userTimezone, JSON.stringify(actualMsg), 0, chatId, uid])
        }
        ...
        io.to(getId[0]?.socket_id).emit('push_new_msg', { msg: actualMsg, chatId: chatId })

        // checking if the agent has this chat 
        const getAgentChat = await query(`SELECT * FROM agent_chats WHERE owner_uid = ? AND chat_id = ?`, [
            uid,
            chatId
        ])

        if (getAgentChat.length > 0) {
            const getAgentSocket = await query(`SELECT * FROM rooms WHERE uid = ?`, [getAgentChat[0]?.uid])
            io.to(getAgentSocket[0]?.socket_id).emit('update_conversations', { chats: chats });

            io.to(getAgentSocket[0]?.socket_id).emit('push_new_msg', { msg: actualMsg, chatId: chatId })
        }
...
async function saveWebhookConversation(body, uid) {
    ...
    // images 
    else if (body?.entry[0]?.changes[0]?.value?.messages && body?.entry[0]?.changes[0]?.value?.messages[0]?.image) {
        ...
        if (metaToken) {
            ...
            const fileName = await downloadAndSaveMedia(metaToken, body?.entry[0]?.changes[0]?.value?.messages[0]?.image?.id)
        ...
        botWebhook(body?.entry[0]?.changes[0]?.value?.messages[0]?.image?.caption || "aU1uLzohPGMncyrwlPIb", uid, body?.entry[0]?.changes[0]?.value?.contacts[0]?.wa_id, body?.entry[0]?.changes ? body?.entry[0]?.changes[0]?.value?.contacts[0]?.profile?.name : "NA")
    }

    // video 
    else if (body?.entry[0]?.changes[0]?.value?.messages && body?.entry[0]?.changes[0]?.value?.messages[0]?.video) {
        const getUser = await query(`SELECT * FROM user WHERE uid = ?`, [uid])
        ...
        if (metaToken) {
            const fileName = await downloadAndSaveMedia(metaToken, body?.entry[0]?.changes[0]?.value?.messages[0]?.video?.id)
    ...
    // document 
    else if (body?.entry[0]?.changes[0]?.value?.messages && body?.entry[0]?.changes[0]?.value?.messages[0]?.document) {
        const getUser = await query(`SELECT * FROM user WHERE uid = ?`, [uid])
        ...
        if (metaToken) {
            const fileName = await downloadAndSaveMedia(metaToken, body?.entry[0]?.changes[0]?.value?.messages[0]?.document?.id)
        ...
        botWebhook(body?.entry[0]?.changes[0]?.value?.messages[0]?.document?.caption || "aU1uLzohPGMncyrwlPIb", uid, body?.entry[0]?.changes[0]?.value?.contacts[0]?.wa_id, body?.entry[0]?.changes ? body?.entry[0]?.changes[0]?.value?.contacts[0]?.profile?.name : "NA")
    }


    // audio 
    else if (body?.entry[0]?.changes[0]?.value?.messages && body?.entry[0]?.changes[0]?.value?.messages[0]?.audio) {
        const getUser = await query(`SELECT * FROM user WHERE uid = ?`, [uid])
        ...
        if (metaToken) {
            const fileName = await downloadAndSaveMedia(metaToken, body?.entry[0]?.changes[0]?.value?.messages[0]?.audio?.id)
    ...
    // adding reactions
    else if (body?.entry[0]?.changes[0]?.value?.messages && body?.entry[0]?.changes[0]?.value?.messages[0]?.reaction) {
        const chatId = convertNumberToRandomString(body?.entry[0]?.changes[0]?.value?.contacts[0]?.wa_id, body?.entry[0]?.changes ? body?.entry[0]?.changes[0]?.value?.contacts[0]?.profile?.name : "NA")
        const filePath = `${__dirname}/../conversations/inbox/${uid}/${chatId}.json`
        updateMessageObjectInFile(filePath, body?.entry[0]?.changes[0]?.value?.messages[0]?.reaction?.message_id, "reaction", body?.entry[0]?.changes[0]?.value?.messages[0]?.reaction?.emoji);
        ...
        io.to(getId[0]?.socket_id).emit('push_new_reaction', { reaction: body?.entry[0]?.changes[0]?.value?.messages[0]?.reaction?.emoji, chatId: chatId, msgId: body?.entry[0]?.changes[0]?.value?.messages[0]?.reaction?.message_id })
        ...
        if (getAgentChat.length > 0) {
            ...
            io.to(getAgentSocket[0]?.socket_id).emit('push_new_reaction', { reaction: body?.entry[0]?.changes[0]?.value?.messages[0]?.reaction?.emoji, chatId: chatId, msgId: body?.entry[0]?.changes[0]?.value?.messages[0]?.reaction?.message_id })
        }
    }

    // quick reply button 
    else if (body?.entry[0]?.changes[0]?.value?.messages && body?.entry[0]?.changes[0]?.value?.messages[0]?.interactive?.button_reply) {
        saveMessage(body, uid, "text", {
            "type": "text",
            "text": {
                "preview_url": true,
        ...
        botWebhook(body?.entry[0]?.changes[0]?.value?.messages[0]?.interactive?.button_reply?.title || "aU1uLzohPGMncyrwlPIb", uid, body?.entry[0]?.changes[0]?.value?.contacts[0]?.wa_id, body?.entry[0]?.changes ? body?.entry[0]?.changes[0]?.value?.contacts[0]?.profile?.name : "NA")
    }

    // updating delivery status 
    else if (body?.entry[0]?.changes[0]?.value?.statuses && body?.entry[0]?.changes[0]?.value?.statuses[0]?.id) {
        const metaMsgId = body?.entry[0]?.changes[0]?.value?.statuses[0]?.id

        // console.log(`update msg:-`, JSON.stringify(body))

        const chatId = convertNumberToRandomString(body?.entry[0]?.changes[0]?.value?.statuses[0]?.recipient_id, body?.entry[0]?.changes || "NA")

        const filePath = `${__dirname}/../conversations/inbox/${uid}/${chatId}.json`
        updateMessageObjectInFile(filePath, metaMsgId, "status", body?.entry[0]?.changes[0]?.value?.statuses[0]?.status);

        const io = getIOInstance();

        const getId = await query(`SELECT * FROM rooms WHERE uid = ?`, [uid])

        io.to(getId[0]?.socket_id).emit('update_delivery_status', { chatId: chatId, status: body?.entry[0]?.changes[0]?.value?.statuses[0]?.status, msgId: body?.entry[0]?.changes[0]?.value?.statuses[0]?.id })
        ...
        if (getAgentChat.length > 0) {
            ...
            io.to(getAgentSocket[0]?.socket_id).emit('update_delivery_status', { chatId: chatId, status: body?.entry[0]?.changes[0]?.value?.statuses[0]?.status, msgId: body?.entry[0]?.changes[0]?.value?.statuses[0]?.id })
        }
        ...
        if (body?.entry[0]?.changes[0]?.value?.statuses[0]?.status === 'failed') {
            ...
            await query(`UPDATE broadcast_log SET delivery_status = ? WHERE meta_msg_id = ?`, [body?.entry[0]?.changes[0]?.value?.statuses[0]?.status, metaMsgId])
        }
    }

    // list reply button 
    else if (body?.entry[0]?.changes[0]?.value?.messages && body?.entry[0]?.changes[0]?.value?.messages[0]?.interactive?.list_reply) {
        saveMessage(body, uid, "text", {
            "type": "text",
            "text": {
                "preview_url": true,
...
function updateMetaTempletInMsg(uid, savObj, chatId, msgId) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const userTimezone = getCurrentTimestampInTimeZone(getUser[0]?.timezone || Date.now() / 1000)
            const finalSaveMsg = { ...savObj, metaChatId: msgId, timestamp: userTimezone }
            ...
            await query(`UPDATE chats SET last_message_came = ?, last_message = ?, is_opened = ? WHERE chat_id = ?`,
            ...
            io.to(getId[0]?.socket_id).emit('push_new_msg', { msg: finalSaveMsg, chatId: chatId })
...
function sendAPIMessage(obj, waNumId, waToken) {
    return new Promise(async (resolve) => {
        try {
            ...
            resolve({ success: true, message: "Message sent successfully!", data: data?.messages[0] })
    ...
    return new Promise(async (resolve) => {
        try {
            ...
            if (data?.messages[0]?.id) {
                const userTimezone = getCurrentTimestampInTimeZone(getUser[0]?.timezone || Date.now() / 1000)
                const finalSaveMsg = { ...savObj, metaChatId: data?.messages[0]?.id, timestamp: userTimezone }
                ...
                if (getAgentChat.length > 0) {
                    ...
                    io.to(getAgentSocket[0]?.socket_id).emit('push_new_msg', { msg: finalSaveMsg, chatId: chatId })
                }
            }
...
async function sendMetatemplet(toNumber, business_phone_number_id, token, template, example) {
    ...
    if (headerFormat === "IMAGE" && getHeader.length > 0) {
        const getMedia = await query(`SELECT * FROM meta_templet_media WHERE templet_name = ?`, [template?.name])
            ...
            parameters: [
                {
                    type: "image",
                    image: {
                        link: getMedia.length > 0 ? `${process.env.FRONTENDURI}/media/${getMedia[0]?.file_name}` : getHeader[0].example?.header_handle[0],
                    },
                },
            ],
        });
